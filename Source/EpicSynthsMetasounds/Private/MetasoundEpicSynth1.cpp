...
#define SYNTH_DEBUG_MODE 0
	...
	FEpicSynth1Voice::FEpicSynth1Voice()
		...
		, bIsActive(false)
	{
		CurrentPatchType[0] = EMetasoundSynthLFOPatchType::PatchToNone;
		CurrentPatchType[1] = EMetasoundSynthLFOPatchType::PatchToNone;

		CurrentModPatchType = EMetasoundSynthModEnvPatch::PatchToNone;
	...
	void FEpicSynth1Voice::Init(FEpicSynth1* InParentSynth, const int32 InVoiceId)
	{
		...
		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToOscFreq];
		...
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToOscFreq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToFilterFreq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		...
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToFilterFreq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToFilterQ];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		Patch->Destinations.Add(Filter_StateVar_Q);
		Patch->Destinations.Add(Filter_Ladder_Q);
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToFilterQ");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToLFO1Gain];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		Patch->Destinations.Add(LFO1Gain);
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToLFO1Gain");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToLFO2Gain];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		Patch->Destinations.Add(LFO2Gain);
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToLFO2Gain");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToLFO1Freq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		Patch->Destinations.Add(LFO1Freq);
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToLFO1Freq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvPatches[(int32)EMetasoundSynthModEnvPatch::PatchToLFO2Freq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceEnv;
		Patch->Destinations.Add(LFO1Freq);
		Patch->SetName("EMetasoundSynthModEnvPatch::PatchToLFO2Freq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToOscFreq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(OscilFreqDest[0]);
		Patch->Destinations.Add(OscilFreqDest[1]);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToOscFreq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToFilterFreq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(Filter_OnePole_Freq);
		Patch->Destinations.Add(Filter_StateVar_Freq);
		Patch->Destinations.Add(Filter_Ladder_Freq);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToFilterFreq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToFilterQ];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(Filter_StateVar_Q);
		Patch->Destinations.Add(Filter_Ladder_Q);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToFilterQ");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToLFO1Gain];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(LFO1Gain);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToLFO1Gain");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToLFO2Gain];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(LFO2Gain);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToLFO2Gain");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToLFO1Freq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(LFO1Freq);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToLFO1Freq");
		ModMatrix->AddPatch(VoiceId, Patch);

		Patch = &ModEnvBiasPatches[(int32)EMetasoundSynthModEnvBiasPatch::PatchToLFO2Freq];
		Patch->bEnabled = false;
		Patch->Source = ModEnv_ModSourceBiasEnv;
		Patch->Destinations.Add(LFO1Freq);
		Patch->SetName("EMetasoundSynthModEnvBiasPatch::PatchToLFO2Freq");
		ModMatrix->AddPatch(VoiceId, Patch);
		...
		for (int32 i = 0; i < NumLFOs; ++i)
		{
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToGain];
			Patch->bEnabled = false;
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToOscFreq];
			Patch->bEnabled = false;
			Patch->Source = NormalPhasePatchSource;
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToOscPulseWidth];
			Patch->bEnabled = false;
			Patch->Source = NormalPhasePatchSource;
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToFilterFreq];
			Patch->bEnabled = false;
			Patch->Source = NormalPhasePatchSource;
			Patch->Destinations.Add(Filter_OnePole_Freq);
			Patch->Destinations.Add(Filter_StateVar_Freq);
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToFilterQ];
			Patch->bEnabled = false;
			Patch->Source = NormalPhasePatchSource;
			Patch->Destinations.Add(Filter_StateVar_Q);
			Patch->Destinations.Add(Filter_Ladder_Q);
			...
			Patch = &LFOPatches[i][(int32)EMetasoundSynthLFOPatchType::PatchToOscPan];
			Patch->bEnabled = false;
			Patch->Source = NormalPhasePatchSource;
		...
		Patch = &LFOPatches[0][(int32)EMetasoundSynthLFOPatchType::PatchLFO1ToLFO2Frequency];
		Patch->bEnabled = false;
		...
		Patch = &LFOPatches[0][(int32)EMetasoundSynthLFOPatchType::PatchLFO1ToLFO2Gain];
		Patch->bEnabled = false;
		Patch->Source = LFO[0].GetModSourceNormalPhase();
		Patch->Destinations.Add(LFO2Gain);
		Patch->SetName("ELFOPatch::PatchLFO1ToLFO2Frequency");
	...
	FPatchSource FEpicSynth1Voice::GetPatchSource(EMetasoundSynth1PatchSource PatchSource)
	{
		switch (PatchSource)
		{
			case EMetasoundSynth1PatchSource::LFO1:
				return LFO[0].GetModSourceNormalPhase();

			case EMetasoundSynth1PatchSource::LFO2:
				return LFO[1].GetModSourceNormalPhase();

			case EMetasoundSynth1PatchSource::Envelope:
				return ModEnv.GetModSourceEnv();

			case EMetasoundSynth1PatchSource::BiasEnvelope:
	...
	void FEpicSynth1Voice::GetPatchDestinations(EMetasoundSynth1PatchDestination PatchDestination, TArray<FPatchDestination>& Destinations)
	{
		...
		{
			case EMetasoundSynth1PatchDestination::Osc1Gain: 
				Destinations.Add(Oscil[0].GetModDestGain()); 
				return;

			case EMetasoundSynth1PatchDestination::Osc1Frequency:
				Destinations.Add(Oscil[0].GetModDestFrequency());
				return;

			case EMetasoundSynth1PatchDestination::Osc1Pulsewidth:
				Destinations.Add(Oscil[0].GetModDestPulseWidth());
				return;

			case EMetasoundSynth1PatchDestination::Osc2Gain:
				Destinations.Add(Oscil[1].GetModDestGain());
				return;

			case EMetasoundSynth1PatchDestination::Osc2Frequency:
				Destinations.Add(Oscil[1].GetModDestFrequency());
				return;

			case EMetasoundSynth1PatchDestination::Osc2Pulsewidth:
				Destinations.Add(Oscil[1].GetModDestPulseWidth());
				return;

			case EMetasoundSynth1PatchDestination::FilterFrequency:
			...
			case EMetasoundSynth1PatchDestination::FilterQ:
			...
			case EMetasoundSynth1PatchDestination::Gain:
				Destinations.Add(Amp.GetModDestGainScale());
				return;

			case EMetasoundSynth1PatchDestination::Pan:
				Destinations.Add(Amp.GetModDestPan());
				return;

			case EMetasoundSynth1PatchDestination::LFO1Frequency:
				Destinations.Add(LFO[0].GetModDestFrequency());
				return;

			case EMetasoundSynth1PatchDestination::LFO1Gain:
				Destinations.Add(LFO[0].GetModDestGain());
				return;

			case EMetasoundSynth1PatchDestination::LFO2Frequency:
				Destinations.Add(LFO[1].GetModDestFrequency());
				return;

			case EMetasoundSynth1PatchDestination::LFO2Gain:
	...
	void FEpicSynth1Voice::SetLFOPatch(const int32 InLFOIndex, const EMetasoundSynthLFOPatchType InPatchType)
	{
		...
		if (InPatchType != EMetasoundSynthLFOPatchType::PatchToNone)
		{
	...
	void FEpicSynth1Voice::SetEnvModPatch(const EMetasoundSynthModEnvPatch InPatchType)
	{
		...
		if (InPatchType != EMetasoundSynthModEnvPatch::PatchToNone)
		{
	...
	void FEpicSynth1Voice::SetEnvModBiasPatch(const EMetasoundSynthModEnvBiasPatch InPatchType)
	{
		...
		if (InPatchType != EMetasoundSynthModEnvBiasPatch::PatchToNone)
		{
	...
	FEpicSynth1::FEpicSynth1()
		...
		, FilterAlgorithm(EMetasoundSynthFilterAlgorithm::OnePole)
		, bIsUnison(false)
	...
	void FEpicSynth1::NoteOn(const uint32 InMidiNote, const float InVelocity, const float Duration)
	{
		...
		else
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Not enough voices left."));
				return;
			}
	...
	void FEpicSynth1::SwitchFilter()
	{
		for (int32 VoiceId = 0; VoiceId < Voices.Num(); ++VoiceId)
		{
			...
			{
				case EMetasoundSynthFilterAlgorithm::OnePole:
				{
					CurrentFilter = &Voices[VoiceId]->OnePoleFilter;
				}
				break;

				case EMetasoundSynthFilterAlgorithm::StateVariable:
				{
					CurrentFilter = &Voices[VoiceId]->StateVarFilter;
				}
				break;

				case EMetasoundSynthFilterAlgorithm::Ladder:
				{
